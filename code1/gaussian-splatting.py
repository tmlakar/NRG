    ...
    for splat in sorted_splats:
        ...
        for x in (x_pixels):
            for y in (y_pixels):
                ...
                # modify the alpha channel of current pixel of image
                if 0 <= int(x) < screen_size[0] and 0 <= int(y) < screen_size[1]:
                    image[int(x), int(y), :3] = (1 - varying_alpha_pixel/255)*image[int(x), int(y), :3] + true_color_splat*varying_alpha_pixel/255
                # if (image[x, y, 3] != 0):
                #     print(image[x, y, 3])
